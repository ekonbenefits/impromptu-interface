...
dotnet test Tests/UnitTestImpromptuInterface/UnitTestImpromptuInterface.csproj -f netcoreapp2.0 --no-build --no-restore --filter=TestCategory!=Performance --logger=trx;LogFileName=testresults.trx --configuration=Debug  
exit $?
